...
mod tests {
    ...
    fn test_calculate_missing_sst() {
        ...
        let chl_ds = driver.create_with_band_type::<f32, _>("", 10, 10, 1).unwrap();
    ...
    fn test_calculate_missing_kd490() {
        ...
        let driver = DriverManager::get_driver_by_name("MEM").unwrap();
        let chl_ds = driver.create_with_band_type::<f32, _>("", 10, 10, 1).unwrap();
        let sst_ds = driver.create_with_band_type::<f32, _>("", 10, 10, 1).unwrap();
        datasets.insert(DatasetType::Chlorophyll, chl_ds);
    ...
    fn test_calculate_with_negative_chlorophyll() {
        ...
        chl_ds.rasterband(1).unwrap().write((0, 0), (2, 2), &mut chl_data).unwrap();
        
        // Write valid SST and Kd values
        let mut sst_data = Buffer::new((2, 2), vec![20.0f32; 4]);
        sst_ds.rasterband(1).unwrap().write((0, 0), (2, 2), &mut sst_data).unwrap();
        let mut kd_data = Buffer::new((2, 2), vec![0.2f32; 4]);
        kd_ds.rasterband(1).unwrap().write((0, 0), (2, 2), &mut kd_data).unwrap();
    ...
    fn test_calculate_with_valid_inputs() {
        ...
        let mut chl_data = Buffer::new((3, 3), vec![0.3f32, 0.5, 0.8, 0.1, 0.2, 0.6, 0.4, 0.5, 0.3]);
        chl_ds.rasterband(1).unwrap().write((0, 0), (3, 3), &mut chl_data).unwrap();
        
        let mut sst_data = Buffer::new((3, 3), vec![15.0f32, 18.0, 20.0, 10.0, 22.0, 19.0, 17.0, 21.0, 16.0]);
        sst_ds.rasterband(1).unwrap().write((0, 0), (3, 3), &mut sst_data).unwrap();
        
        let mut kd_data = Buffer::new((3, 3), vec![0.1f32, 0.15, 0.2, 0.12, 0.18, 0.15, 0.13, 0.16, 0.14]);
        kd_ds.rasterband(1).unwrap().write((0, 0), (3, 3), &mut kd_data).unwrap();
        ...
        for (i, &value) in result.iter().enumerate() {
            if value.is_finite() {
                ...
                assert!(value <= 2000.0, "Pixel {}: value should be within reasonable range", i);
            }
    ...
    fn test_calculate_boundary_sst() {
        ...
        chl_ds.rasterband(1).unwrap().write((0, 0), (3, 1), &mut chl_data).unwrap();
        
        // Write extreme SST values (cold, normal, hot)
        let mut sst_data = Buffer::new((3, 1), vec![-2.0f32, 20.0, 35.0]);
        sst_ds.rasterband(1).unwrap().write((0, 0), (3, 1), &mut sst_data).unwrap();
        
        let mut kd_data = Buffer::new((3, 1), vec![0.1f32; 3]);
        kd_ds.rasterband(1).unwrap().write((0, 0), (3, 1), &mut kd_data).unwrap();
    ...
    fn test_calculate_nan_input_handling() {
        ...
        chl_ds.rasterband(1).unwrap().write((0, 0), (4, 1), &mut chl_data).unwrap();
        
        let mut sst_data = Buffer::new((4, 1), vec![20.0f32, f32::NAN, 20.0, 20.0]);
        sst_ds.rasterband(1).unwrap().write((0, 0), (4, 1), &mut sst_data).unwrap();
        
        let mut kd_data = Buffer::new((4, 1), vec![0.2f32, 0.2, f32::NAN, 0.2]);
        kd_ds.rasterband(1).unwrap().write((0, 0), (4, 1), &mut kd_data).unwrap();
        ...
        assert!(result[0].is_nan(), "Pixel 0: NaN chlorophyll should produce NaN result");
        assert!(result[1].is_nan(), "Pixel 1: NaN SST should produce NaN result");
        assert!(result[2].is_nan(), "Pixel 2: NaN Kd should produce NaN result");
        // Last pixel should be valid
        assert!(result[3].is_finite() && result[3] > 0.0, "Pixel 3: valid inputs should produce finite result");
    }
    ...
    fn test_calculation_formula_spot_check() {
        ...
        chl_ds.rasterband(1).unwrap().write((0, 0), (1, 1), &mut chl_buf).unwrap();
        let mut sst_buf = Buffer::new((1, 1), vec![sst]);
        sst_ds.rasterband(1).unwrap().write((0, 0), (1, 1), &mut sst_buf).unwrap();
        let mut kd_buf = Buffer::new((1, 1), vec![kd]);
        kd_ds.rasterband(1).unwrap().write((0, 0), (1, 1), &mut kd_buf).unwrap();
        ...
        assert!(result[0].is_nan(), "Result should be NaN due to exceeding 2000 threshold");
    }
    ...
    fn test_calculation_within_reasonable_range() {
        ...
        assert!(result[0] <= 2000.0, "Result should be within reasonable range");
    }
}
